    ...
    (->
        if currentViewMode == 0 # save crop mode
            ...
            ((error)->
                ...
                logException("Error Occured on crop  #{error.status} #{error.statusText} INODE => #{currentInode.path} URL => #{resizeAction.url}", "9#{ln()}")
            ))
            ...
            ((error)->
                ...
                logException("Error Occured on resize  #{error.status} #{error.statusText} INODE => #{currentInode.path} URL => #{resizeAction.url}", "9#{ln()}")
            ))
    )
